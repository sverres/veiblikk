...
const VEIBLIKK_webcams = (function () {
  ...
  const make_segments = function (route) {
  ...
  const route_segments_loop = function (route) {
    ...
    setTimeout(function () {
      find_cctvs_in_segment(route)
    }, default_timeout);
  };


  const find_cctvs_in_segment = function (route) {
    ...
    cctv_JSON
      ...
      .cctvCameraList
      .cctvCameraMetadataRecord.map(function (cctv_record) {
  ...
  const store_cctv_point = function (cctv_point, cctv_record, route) {
  ...
  const display_cctvs = function () {
    cctv_locations_on_route.sort(function (distance_1, distance_2) {
    ...
    cctv_locations_on_route.map(function (webcam) {
